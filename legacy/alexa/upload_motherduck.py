...
El biométrico es IP local: generá Parquet con `legacy/alexa/sync_device_respaldo.py`
(o tu propio pipeline) en la PC en la LAN; este script empuja el Parquet hacia MotherDuck.
...
Uso (desde la raíz de talento/):
  ...
  python legacy/alexa/upload_motherduck.py --parquet raw/biometrico_parquet/20260501.parquet
...
from biometrico.schemas.paths import RAW_BIOMETRICO_PARQUET_DIR, stem_export_mes_biometrico
...
def upload_parquet(con: duckdb.DuckDBPyConnection, parquet_path: Path) -> None:
    ...
    for col in required:
        if col not in df.columns:
            sys.exit(
                f"[ERROR] El Parquet no tiene columna '{col}'. "
                "Regenerá el Parquet (p. ej. python legacy/alexa/sync_device_respaldo.py --mes ...)."
            )

    anios = df["anio"].unique().to_list()
    meses = df["mes"].unique().to_list()
    if len(anios) != 1 or len(meses) != 1:
        sys.exit("[ERROR] Se espera un solo mes/año por archivo (un mes por Parquet).")
...
def main() -> None:
    ...
    if args.parquet:
        ...
        pq = RAW_BIOMETRICO_PARQUET_DIR / f"{stem_export_mes_biometrico(args.anio, mes_num)}.parquet"
